...
// 	}
// }
Buffer.prototype['readReal48LE'] = function (index) {
...
Buffer.prototype['readExtendedLE'] = function (index) {
    const bytes = this.slice(index, index + 10).reverse();
    let f;
    let expon;
    expon = ((bytes[0] & 0x7F) << 8) | bytes[1];
    const hiMant = ((bytes[2] & 0x7F) === bytes[2]) ? 0 : 0x80000000 +
        (((bytes[2] & 0x7F) << 24) | (bytes[3] << 16) | (bytes[4] << 8) | bytes[5]);
    const loMant = ((bytes[6] & 0x7F) === bytes[6]) ? 0 : 0x80000000 +
